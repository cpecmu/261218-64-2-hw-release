 ...
 */
public interface DoctorsOnCallRelaxed<D> {

    /**
     * Construct a duty schedule of doctors on duty, so that each day has the
     * number of doctors exactly as desired and each doctor works no more than
     * the given number of extra days, or report that such a schedule does not
     * exist.
     ...
     * @param c the number of extra days that each doctor may work outside of
     ...
     *          no more than {@code c} days outside their available days;
     *          otherwise, {@code null} (when there is no such schedule)
     */
